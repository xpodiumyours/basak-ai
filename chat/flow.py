...
def _profil_isle(text, konusmaci):
    try:
        ...
        if silinen == -1:
            not_ = "Not: Casper hakkındaki tüm profil bilgilerini SİLDİN. Bunu doğrula."
        elif silinen:
            not_ = "Not: profilden %d kayıt sildin (istek: %s). Bunu doğrula." % (silinen, text)
        else:
            yeniler = ogren(motor, text, speaker=konusmaci or "")
            not_ = ""
            if yeniler:
                not_ = "Not: profile yeni bilgi eklendi: %s. Kısaca doğrulayıp sohbete devam et." % "; ".join("%s=%s" % (a, d) for a, d in yeniler)
...
_ARAC_ISARETLERI = (
    "araştır", "arastir", "ara bakalım", "ara bakalim", "internetten", "güncel", "guncel",
    "son durum", "haber", "fiyat", "kaç para", "kac para", "kaça", "kaca", "ne kadar",
    "rakip", "pazar", "hedef kitle", "müşteri", "musteri", "trend", "piyasa", "kur",
    "dolar", "euro", "hava durumu", "hava nasıl", "hava nasil", "site", "sayfa", "link",
    "http://", "https://", "www.", "dosya", "klasör", "klasor", "belge", "masaüstü",
    "masaustu", "indirilenler", "listele", "oku", "içinde ne", "icinde ne", "diskimde",
    "bilgisayarımda", "bilgisayarimda", "planda", "belgede", "dokümanda", "dokumanda",
    "notlarda", "hangi dosyada", "yaz", "kaydet", "not al", "dosya oluştur", "dosya olustur",
    "hatırlat", "hatirlat", "ajanda", "bugün ne var", "bugun ne var", "görev", "gorev",
    "yapılacak", "yapilacak", "tamamladım", "tamamladim", "vixrex", "numeramatch", "xses",
    "başak projesi", "basak projesi", "commit", "dal ", "branch", "ne durumda", "durumu ne",
    "ne oldu", "değişti", "degisti", "ekran görüntüsü", "ekran goruntusu", "görsel", "gorsel",
    "resim", "fotoğraf", "fotograf", ".png", ".jpg", ".jpeg", "şu görüntü", "su goruntu",
)
...
def _kaydet(text, cevap, kaynak, gecmis, js_callback, konusmaci):
    gecmis += [{"role": "user", "content": text, "oturum": ctx.OTURUM_ID}, {"role": "assistant", "content": cevap, "oturum": ctx.OTURUM_ID}]
    ...
    if motor and cevap:
        try:
            motor.episodik_kaydet(text, cevap, speaker=konusmaci or "", onem=ctx.onem_puanla(text))
        except Exception as e:
...
def mesaj_isle(text, brain, system_prompt, js_callback, tools=None):
    ...
    if not modeller and not brain.bulut_musait():
        js_callback("BasakUI.error(" + _j("Hicbir beyin yok: Ollama kapali ve bulut anahtarlari da hazir degil") + ")")
        return
    ...
    if modeller:
        ...
        model = None
    gecmis = ctx.temizle_history([m for m in ctx.yukle(ctx.HISTORY_FILE, []) if m.get("role") != "system"])
    ...
    if yayin is not None:
        try:
            ...
            if tam:
                _kaydet(text, tam, kaynak or "bulut", gecmis, js_callback, konusmaci)
                return
        except AracIstegi:
    ...
    try:
        yanit, kaynak = brain.cevapla(mesajlar, model, tools=(tools if arac_acik else None))
    except Exception as e:
        hata = str(e)
        if "429" in hata or "rate" in hata.lower():
            js_callback("BasakUI.error(" + _j("Cok fazla istek, biraz bekle") + ")")
        else:
            js_callback("BasakUI.error(" + _j("Beyin hatasi: " + hata[:150]) + ")")
        return
    ...
    if tool_calls and tools:
        ...
        cevap, kosan = arac_dongusu(tool_calls, mesajlar, brain, model, js_callback, calistir, tools=tools)
