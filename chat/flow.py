...
def _konusmaci_ayir(text):
    ...
    if not eslesme: return text,None
...
def _profil_isle(text,konusmaci):
    try:
        ...
        logger.warning("Profil islenemedi: %s",e); return "",""

_ARAC_ISARETLERI=(
"araştır","arastir","ara bakalım","ara bakalim","internetten","güncel","guncel","son durum","haber","fiyat","kaç para","kac para","kaça","kaca","ne kadar","rakip","pazar","hedef kitle","müşteri","musteri","trend","piyasa","kur","dolar","euro","hava durumu","hava nasıl","hava nasil","site","sayfa","link","http://","https://","www.",
"dosya","klasör","klasor","belge","masaüstü","masaustu","indirilenler","listele","oku","içinde ne","icinde ne","diskimde","bilgisayarımda","bilgisayarimda","planda","belgede","dokümanda","dokumanda","notlarda","hangi dosyada",
"yaz","kaydet","not al","dosya oluştur","dosya olustur",
"hatırlat","hatirlat","ajanda","bugün ne var","bugun ne var","görev","gorev","yapılacak","yapilacak","tamamladım","tamamladim",
"aç","ac","başlat","baslat","çalıştır","calistir",
"vixrex","numeramatch","xses","başak projesi","basak projesi","commit","dal ","branch","ne durumda","durumu ne","ne oldu","değişti","degisti",
"ekran görüntüsü","ekran goruntusu","görsel","gorsel","resim","fotoğraf","fotograf",".png",".jpg",".jpeg","şu görüntü","su goruntu")
...
def mesaj_isle(text,brain,system_prompt,js_callback,tools=None):
    ...
    arac_acik=bool(tools) and _arac_gerek(text); mesajlar=_baglam_kur(text,system_prompt,konusmaci,arac_acik)
