...
DURUM_ALANI = ("query", "url", "path", "folder", "proje", "text", "task_id", "uygulama")
    ...
    for tur in range(tur_siniri):
        ...
        for call in tool_calls:
            ...
            if ad not in TANINMIS_TOOLLAR:
                logger.info("Bilinmeyen arac atlandi: %s",ad); continue
            js_callback("BasakUI.toolStatus("+_j(_durum(ad,args))+")")
            net=sonucu_donustur(calistir(ad,args)); tur_sonuclari.append((ad,net))
        ...
        try: yanit,_kaynak=brain.cevapla(expanded,model,tools=sonraki)
            ...
            logger.warning("Arac turu sonrasi cevap alinamadi: %s",e); break
