...
DURUM_METNI={"web_search":"İnternette aranıyor","sayfa_oku":"Sayfa okunuyor","read_file":"Dosya okunuyor","list_files":"Klasör listeleniyor","git_durum":"Proje durumu ölçülüyor","belge_ara":"Belgelerde aranıyor","dosya_bilgi":"Dosya bilgisi ölçülüyor","icerik_ara":"Kodda aranıyor","write_file_tool":"Dosya yazılıyor","get_reminders":"Hatırlatmalar kontrol ediliyor","add_task":"Görev ekleniyor","list_tasks":"Görevler listeleniyor","complete_task":"Görev tamamlanıyor","ac_uygulama":"Uygulama açılıyor","image_analyze":"Görüntü inceleniyor"}
DURUM_ALANI=("query","url","path","folder","proje","sorgu","text","task_id","uygulama")
def _j(obj):return json.dumps(obj,ensure_ascii=False)
def parse_args(ham):
    if isinstance(ham,dict):return ham
    try:
        cozulmus=json.loads(ham or "{}");return cozulmus if isinstance(cozulmus,dict) else {}
    except (ValueError,TypeError):return {}
def _durum(tool_name,args):
    etiket=DURUM_METNI.get(tool_name,"Çalışıyor");detay=""
    for alan in DURUM_ALANI:
        if (args or {}).get(alan):detay=str(args[alan])[:70];break
    return "%s: %s"%(etiket,detay) if detay else etiket+"..."
def sonucu_donustur(sonuc):
    if isinstance(sonuc,dict):
        if sonuc.get("error"):return "Hata: %s"%sonuc["error"]
        return str(sonuc.get("result",""))
    return str(sonuc)
def _kaynak_satiri(cevap,kaynaklar):
    if not kaynaklar or not cevap:return cevap
...
def arac_dongusu(tool_calls,mesajlar,brain,model,js_callback,calistir,tools=None,tur_siniri=TUR_SINIRI):
    ...
    for tur in range(tur_siniri):
        ...
        for call in tool_calls:
            ...
            if ad not in TANINMIS_TOOLLAR:logger.info("Bilinmeyen arac atlandi: %s",ad);continue
        ...
        try:yanit,_kaynak=brain.cevapla(expanded,model,tools=sonraki)
