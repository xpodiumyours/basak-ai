...
def test_vercel_sohbet_bitir_beklemeden_sse_akisi_verir(monkeypatch):
    import asyncio
    import json
    import threading
    ...
    def _sahte_mesaj_isle(_metin, _beyin, _sistem, js_callback,
        ...
        js_callback('BasakUI.parca(" ikinci")')
        js_callback('BasakUI.bitir("ilk ikinci", "sahte")')
    ...
    async def _senaryo():
        ...
        # Model hala bloklu: ilk olay final beklenmeden geldi.
        ...
        return [
            ...
            for parca in parcalar if parca.strip()
        ]

    olaylar = asyncio.run(_senaryo())
    assert [o["tur"] for o in olaylar] == [
        "thinking", "parca", "parca", "bitir"
    ]
    assert olaylar[-1]["cevap"] == "ilk ikinci"


def test_vercel_web_sse_akisini_aninda_gosterir():
    metin = open("web/app.js", encoding="utf-8").read()
    assert "async function sseAkisiniOku" in metin
