"""tools/web_search.py — DuckDuckGo web araması + güvenli sayfa okuma.

Kullanıcı sorgusu kelime/niyet kalıplarıyla farklı bir araca yönlendirilmez.
Arama sonuçları URL'leriyle birlikte döner; hangi sayfanın okunacağına model karar verir.
"""
...
def web_search(query: str) -> dict:
    """DuckDuckGo'da genel web araması yapar."""
    if not query or not query.strip():
        return {"error": "Arama sorgusu boş olamaz"}
    return _duckduckgo_ara(query.strip())


def _duckduckgo_ara(query):
    """DuckDuckGo'da arama yapar ve sonuç URL'lerini korur."""
    try:
        from ddgs import DDGS

        with DDGS() as ddgs:
            results = list(ddgs.text(query, region="tr-tr", max_results=8))
        ...
        for i, r in enumerate(results, 1):
            baslik = _temizle(r.get("title", ""))
            body = _temizle(r.get("body", ""))
            adres = (r.get("href") or r.get("url") or "").strip()
            satirlar = ["[%d] %s" % (i, baslik or "Sonuç")]
            if adres:
                satirlar.append("URL: %s" % adres)
            if body:
...
def _temizle(text):
    """Sonuç metninde yalnız gereksiz boşlukları temizler."""
