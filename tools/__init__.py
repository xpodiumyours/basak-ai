...
Araç adı tools.definitions.TOOLS listesinden türetilen beyaz listede
olmak zorundadır. Her aracın gerçek sınırı kendi modülünde veya aşağıdaki
dar bağlayıcıda uygulanır; onay kuyruğu/izin katmanı yoktur.
"""
...
def _knowledge_altinda_mi(yol):
    """Model-facing yazma yalnız gerçek knowledge/ kökünde kalır."""
    try:
...
def calistir(tool_name, args):
    ...
    try:
        ...
        if tool_name == "adres_kontrol":
            from tools import address_check
            return address_check.adres_kontrol(str(args.get("url", "")))
        ...
        if tool_name == "git_gecmis":
            ...
            return olcum.git_gecmis(
                str(args.get("proje", "")), args.get("dosya"),
                args.get("adet", 10))
        ...
        if tool_name == "git_degisenler":
            ...
            return olcum.git_degisenler(
                str(args.get("proje", "")),
                str(args.get("taban", "origin/master") or "origin/master"))
        ...
        if tool_name == "icerik_ara":
            ...
            return olcum.icerik_ara(
                str(args.get("proje", "")),
                str(args.get("sorgu", "")), args.get("uzanti"))
        ...
        if tool_name == "github_durum":
            from tools import github_status
            return github_status.github_durum(
                str(args.get("proje", "")),
                str(args.get("islem", "pr_list")),
                args.get("pr_numarasi"),
                str(args.get("durum", "open")))

        if tool_name == "testleri_kos":
            from tools import test_runner
            return test_runner.testleri_kos(str(args.get("proje", "")))
